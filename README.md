# ディスプレイ＠OFF

**ワンクリックでディスプレイをスタンバイモードに移行するシンプルツール**

ディスプレイ＠OFF は、Windows の電源プランで設定される「ディスプレイの電源を切る」機能と同等の動作を、ワンクリックで実行できるデスクトップアプリケーションです。席を離れる際や、PC を稼働させたままディスプレイのみを消したい場合に便利です。

<img width="252" height="189" alt="image" src="https://github.com/user-attachments/assets/447f1608-9311-4999-bcf4-664a6d8f0c2d" />


## 🚀 主な特徴

- **ワンクリック操作**: 起動するだけでディスプレイを即座にオフにします。
- **カウントダウン機能**: 5秒のカウントダウン後に自動実行します。操作を取り消す余裕があります。
- **キャンセル機能**: ESC キーまたはキャンセルボタンで、いつでもカウントダウンを中止できます。
- **軽量・高速起動**: ネイティブ AOT コンパイルにより、瞬時に起動します。
- **自動更新**: Windows へサインインした際にバックグラウンドで更新を確認し、新しいバージョンが公開されていれば自動で適用します。常に最新の機能と安定性を提供します。

## 📥 インストール方法

1. **[公式サイト](https://totd.kagayoi.com/)** から最新の [`Setup.exe`](https://totd.kagayoi.com/TurnOffTheDisplay-win-Setup.exe) をダウンロードします。
2. ダウンロードした `Setup.exe` を実行してインストールします。
3. インストール完了後、デスクトップやスタートメニューからディスプレイ＠OFF を起動できます。

## ⚙️ システム要件

- **OS**: Windows 10 / 11 (x64)
- **権限**: 一般ユーザー権限で動作（管理者権限は不要です）

## 🏷️ 種別

フリーソフト（MIT License）

## 📧 連絡先

- メール: 1llum1n4t1@duck.com
- GitHub Issues: https://github.com/1llum1n4t1s/TurnOffTheDisplay/issues

## 📖 使い方

1. アプリケーションを起動します。
2. 5秒のカウントダウンが開始されます。
3. カウントダウン終了後、ディスプレイが自動的にスタンバイモードに移行します。
4. 操作をキャンセルしたい場合は、**ESC キー**または**「キャンセル」ボタン**をクリックしてください。

## 🗑️ アンインストール方法

Windows の「設定」→「アプリ」→「インストールされているアプリ」から「ディスプレイ＠OFF」を選択し、「アンインストール」をクリックしてください。

## 🔄 更新履歴

### Ver 2.0.35 (2026/08)

- アプリアイコンを水色基調のネオングラスデザインへ刷新

### Ver 2.0.33 (2026/07)

- 更新ファイルの配信元を新ドメインへ移行し、自動更新の安定性を改善
- 通知の表示元がアプリ本体として正しく識別されるよう修正

### Ver 2.0.32 (2026/07)

- スタートメニューのショートカットを直下へ配置し、「ゆろち」フォルダを新規作成しないよう改善
- 既存の「ゆろち」フォルダ内ショートカットを、スタートメニュー直下へ自動移行
- 更新ファイルの配信反映を改善

### Ver 2.0.18 (2026/03)

- Avalonia UI に移行（WPF から）
- ネイティブ AOT コンパイル対応
- MVVM パターン導入 (CommunityToolkit.Mvvm)
- Windows 起動時の自動アップデートチェック機能追加
- Velopack によるインストーラー・自動更新基盤の刷新
- アプリ表示名を「ディスプレイ＠OFF」に変更
- ディスプレイ OFF 処理の信頼性を改善
- 起動時のクラッシュを修正

### Ver 2.00 (2025/07/19)

- WPF アプリケーションとして全面的に再作成
- カウントダウン機能とキャンセル機能を追加
- UI/UX を改善

### Ver 1.01 (2011/03/23)

- 特定環境下での動作不具合を修正
- Delphi XE 環境への移行

### Ver 1.00 (2008/06/21)

- 初回リリース

## 📄 ライセンス

ディスプレイ＠OFF は MIT License の下で公開されています。

Copyright (c) 2008-2026 ゆろち

詳細は [LICENSE](LICENSE) ファイルをご参照ください。
